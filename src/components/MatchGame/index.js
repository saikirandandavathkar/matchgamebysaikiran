...
class MatchGame extends Component {
  state = {
    status: tabsList[0].tabId,
    updatedSeconds: 60,
    imgLink: imagesList[0].imageUrl,
    updatedScore: 0,
    scoreCard: false,
  }

  componentDidMount() {
    this.timerId = setInterval(this.decrementingSeconds, 1000)
  }

  decrementingSeconds = () => {
    const {updatedSeconds} = this.state
    if (updatedSeconds === 0) {
      this.setState({scoreCard: true})
      clearInterval(this.timerId)
    } else {
      this.setState(prevState => ({
        updatedSeconds: prevState.updatedSeconds - 1,
      }))
    }
  }
  ...
  updateImgUI = () => {
    ...
    this.setState({imgLink: obj.imageUrl})
  }

  updatedScore = thumbnailUrl => {
    const {imgLink} = this.state
    console.log(imgLink)
    console.log(thumbnailUrl)

    const thumbnailObj = imagesList.filter(
      eachList => eachList.thumbnailUrl === thumbnailUrl,
    )
    const {imageUrl} = thumbnailObj[0]

    if (imgLink === imageUrl) {
      this.setState(
        prevState => ({updatedScore: prevState.updatedScore + 1}),
        this.updateImgUI,
      )
    } else {
      this.setState({scoreCard: true})
  ...
  getUpdatedSeconds = () => {
    const {updatedSeconds} = this.state
    return `${updatedSeconds} sec`
  }

  displayUIOfHeader = () => {
    const {updatedScore} = this.state
    return (
        ...
        <ul className="timer-container">
          <li>
            <p className="score-heading">
              score:
              <span className="score"> {updatedScore} </span>
            </p>
          </li>
          <li>
            <img
          ...
          <p className="timer"> {this.getUpdatedSeconds()} </p>
        </ul>
      </nav>
    )
  }
  ...
  displayImgUI = () => {
    ...
    return (
      <div className="displayImage-container">
        <img src={imgLink} alt="match" className="displayImage" />
      </div>
    )
  }
  ...
  getStatusList = () => {
    ...
    const filteredList = imagesList.filter(
      eachList => eachList.category === status,
    )
    return filteredList
  }
  ...
  displayScoreCard = () => {
    ...
    return (
      <div className="scoreCard-container">
        ...
        <h1 className="score-heading"> {updatedScore} </h1>
        <button
          type="button"
        ...
        <ul className="tabsList-container">
          {tabsList.map(eachList => (
            <TabsList
              ...
              isItActivated={eachList.tabId === status}
            />
          ))}
        </ul>
        <ul className="imagesList-container">
          {statusList.map(eachImgList => (
            <ImagesList
              key={eachImgList.id}
              eachImgDetails={eachImgList}
              updatedScore={this.updatedScore}
            />
          ))}
        </ul>
      </>
  ...
  render() {
    ...
    return (
      <div className="container">
        {this.displayUIOfHeader()}
        {scoreCard ? this.displayScoreCard() : this.displayEntireUI()}
      </div>
    )
  }
}
